...
for i = 0:15:360
    ...
    %Rotate the image with 15 degree step by step, detecting modified keypoints
    [kps_sift_modify,des_sift_modify] = vl_sift(I_gray_single_modify,'PeakThresh', peak_thresh, 'edgethresh', edge_thresh);
    %Rotate the keypoints from the original image
    kps_sift_rotate = Kps_rotate_fun(size(I,2), size(I,1),kps_sift,i);
    ...
    repeatability = count_matches / size(kps_sift_rotate,2);
    y = [y,repeatability];
    x = [x,i];
    ...
    %Rotate the keypoints from the original image
    kps_surf_rotate = Kps_rotate_fun(size(I,2), size(I,1),kps_surf,i);
    ...
    %Compute repeatability by using the matches number
    repeatability = count_matches / size(kps_surf_rotate,2);
